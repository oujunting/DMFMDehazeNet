...
@MODELS.register_module()
class TwoStageBasicRestorer(BaseModel):
    """Basic model for image restoration.
    ...
    def forward_train(self, lq, gt):
        ...
        stage2_output = self.stage2_generator(stage1_output)
        downsample_gt = F.interpolate(gt, scale_factor=0.25, mode='bilinear', align_corners=True)
        loss_final_pix = self.final_loss(stage2_output, gt)
        ...
        outputs = dict(
            losses=losses,
            num_samples=len(gt.data),
            results=dict(lq=lq.cpu(), gt=gt.cpu(), output=stage2_output.cpu()))
        return outputs
    ...
    def forward_test(self,
        ...
        if self.test_cfg is not None and self.test_cfg.get('metrics', None):
            assert gt is not None, (
                'evaluation with metrics must have gt images.')
            results = dict(eval_result=self.evaluate(stage2_output, gt))
        else:
            results = dict(lq=lq.cpu(), output=stage2_output.cpu())
            if gt is not None:
                results['gt'] = gt.cpu()
        ...
        if save_image:
            ...
            if isinstance(iteration, numbers.Number):
                ...
                raise ValueError('iteration should be number or None, '
                                 f'but got {type(iteration)}')
            mmcv.imwrite(tensor2img(stage2_output), save_path)
    ...
    def forward_dummy(self, img):
        ...
        Returns:
            Tensor: Output image.
        """
        stage1_output = self.stage1_generator(img)
        stage2_output = self.stage2_generator(stage1_output)
