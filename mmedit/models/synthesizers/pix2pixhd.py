...
class Pix2PixHD(BaseModel):
    ...
    def merge(self, real_a, fake_b):
        ...
        midgray = (midgray + 1.0) * 0.5
        A = (real_a + 1.0) * 0.5
        fake_b_soft = torch.where(midgray <= 0.5, 2 * A * midgray + A ** 2 * (1 - 2 * midgray), A)
        fake_b_soft = torch.where(midgray > 0.5, 2 * A * (1 - midgray) + (2 * midgray - 1) * torch.sqrt(A), fake_b)
        C0 = fake_b_soft * 2.0 - 1.0

        epsilon = 1e-6  # 小常数，避免零值带来的问题
        real_a = torch.clamp(A, min=epsilon)  # 确保 real_a 不为零或负值
        fake_b = torch.clamp(midgray, min=epsilon)  # 确保 fake_b 不为零或负值

        # 加入增强的乘积模式
        C1 = real_a * torch.sin(fake_b) + real_a * torch.cos(fake_b)
        C1 = C1 * 2.0 - 1.0

        # 加入增强的加法模式
        C2 = torch.sqrt(real_a ** 2 + fake_b ** 2)
        C2 = C2 * 2.0 - 1.0

        # 加入增强的指数模式
        C3 = torch.exp(real_a * fake_b) - 1
        C3 = C3 * 2.0 - 1.0

        # 加入增强的对数模式
        C4 = torch.log(1 + real_a) + torch.log(1 + fake_b)
        C4 = C4 * 2.0 - 1.0

        # 深度模型融合（加权求和或直接相加）
        final_output = 0.25 * C0 + 0.30 * C1 + 0.20 * C2 + 0.15 * C3 + 0.1 * C4
