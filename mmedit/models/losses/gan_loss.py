...
class LKAGANLoss(nn.Module):
    def __init__(self, use_lsgan=True, target_real_label=1.0, target_fake_label=0.0,
                 relativistic=True, average=True):
        super(LKAGANLoss, self).__init__()
    ...
    def forward(self, input, target_is_real=True, opposite=None):
...
@LOSSES.register_module()
class LKAGeneratorLoss(nn.Module):
    def __init__(self, vgg_type='vgg19'):
        super(LKAGeneratorLoss, self).__init__()
        assert vgg_type in ['vgg11', 'vgg13', 'vgg16', 'vgg19']
        vgg = eval(vgg_type)(pretrained=True)
        loss_network = vgg.features[:-2].eval()
        ...
        self.tv_loss = TVLoss()
        # self.RaLSGAN_loss = LKAGANLoss(use_lsgan=True, relativistic=True, average=True)
    ...
    def forward(self, out_images, target_images):
        ...
        return 1e-2 * image_loss + perception_loss + 2e-8 * tv_loss
