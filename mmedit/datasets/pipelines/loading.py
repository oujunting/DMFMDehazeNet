...
@PIPELINES.register_module()
class LoadBlemishPairedImage(LoadImageFromFile):
    def __call__(self, results):
        if self.file_client is None:
            self.file_client = FileClient(self.io_backend, **self.kwargs)

        for k in self.key:
            filepath = str(results[f'{k}_path'])
            img_bytes = self.file_client.get(filepath)
            ...
            results[k] = img
            results[f'{k}_path'] = filepath
            results[f'{k}_ori_shape'] = img.shape

        if len(self.key) == 2 or len(self.key) == 3:
            results['img_a'] = results['A']
            results['img_b'] = results['B']
            results['img_a_path'] = results['A_path']
            results['img_b_path'] = results['B_path']
            results['img_a_ori_shape'] = results['A_ori_shape']
            results['img_b_ori_shape'] = results['B_ori_shape']
        else:
            results['img_a'] = results['A']
            results['img_a_path'] = results['A_path']
            results['img_a_ori_shape'] = results['A_ori_shape']
